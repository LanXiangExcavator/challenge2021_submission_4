...
def run_my_model(model_list, header, recording, config_path):
    model_domain, model_500Hz_10s, model_500Hz_15s, model_250Hz_10s = model_list[0], model_list[1], model_list[2], model_list[3]
    ...
    if config["arch"]["args"]["channel_num"] == 2:
        window_size = 4992
        resample_Fs = 500
    recording3 = copy.deepcopy(recording)
    recording3 = resample(recording3, header, resample_Fs)
    ...
    n_segment = 1
    # slide and cut
    recording3 = slide_and_cut(recording3, n_segment, window_size, resample_Fs, test_time_aug=True)

    if config["arch"]["args"]["channel_num"] == 8:
        leads_index = [0, 1, 6, 7, 8, 9, 10, 11]
        recording3 = recording3[:, leads_index, :]
    recording3[np.isnan(recording3)] = 0
    data = torch.tensor(recording3)
    data = data.to(device, dtype=torch.float)

    if config["arch"]["args"]["channel_num"] == 2:
        output, _ = model_250Hz_10s(data)
    else:
        output = model_250Hz_10s(data)
    prediction3 = torch.sigmoid(output)
    prediction3 = prediction3.detach().cpu().numpy()
    prediction3 = np.expand_dims(np.max(prediction3, axis=0), axis=0)

    prediction = np.mean(np.concatenate([prediction1, prediction2, prediction3], axis=0), axis=0)
    # prediction = np.mean(prediction3, axis=0)
...
def load_model(model_directory, leads):
    ...
    if leads_num == 4:
        checkpoint_path = model_directory + '/lead_' + str(leads_num) + '_pretrain_model_best_250Hz_10s.pth'
    elif leads_num == 2:
        checkpoint_path = model_directory + '/lead_2_model_best.pth'
    else:
        checkpoint_path = model_directory + '/lead_' + str(leads_num) + '_model_best_250Hz_10s.pth'
    model3 = load_my_model(config, checkpoint_path)
    model3.eval()
    print(checkpoint_path)
    ...
    return [model_domain, model1, model2, model3]
